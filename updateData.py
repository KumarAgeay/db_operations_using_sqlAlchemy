...
class UpdateData:
    def updateData(self):
        try:
            ...
            self.user_to_update = self.local_session.query(User).filter(User.username == "Namita").first()
            self.user_to_update.username = "Smita"
            self.user_to_update.email = "smita123@gmail.com"
